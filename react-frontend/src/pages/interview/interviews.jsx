...
function Interviews() {
    const [interviews, setCandidates] = useState([]);
    ...
    const fetchCandidates = useCallback(async () => {
        ...
        try {
            ...
            const response = await api.get("/interview/getall", { params });
            ...
            setError(
                ...
                "Failed to fetch interviews. Please check your network connection and login status."
            );
        } finally {
            setLoading(false);
        }
    ...
    const columns = useMemo(
        () => [
            ...
            columnHelper.accessor("interviewers", {
                header: "Interviewers",
                cell: (info) =>
                    info.getValue()?.map((user) => user.name).join(", ") || "-",
            }),
                                    ...
                                    <tr>
                                        <td colSpan={columns.length} className="px-6 py-12 text-center text-gray-500">
                                            <div className="max-w-xs mx-auto space-y-2">
                                                ...
                                                <p className="font-semibold text-gray-700">No interviews found</p>
                                                <p className="text-xs text-gray-400">
                                                    {searchQuery ? `No results matching "${searchQuery}"` : "There are currently no interviews stored in the system."}
                                                </p>
                                            </div>
                                        </td>
                                    </tr>
                            ...
                            <div className="text-sm text-gray-600">
                                {totalCandidates > 0 ? (
                                    <span>
                                        ...
                                        <span className="font-semibold text-gray-900">{totalCandidates}</span> interviews
                                    </span>
                                ) : (
